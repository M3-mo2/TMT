...
routers (FSM-filtered routers first, the catch-all common router last), the
user gate middleware, and injects every service as dispatcher workflow data
(available to handlers as named parameters). The bot never imports telethon
directly; Telegram-facing work goes through core services and the tg modules
passed in here.

Workflow data keys: ``config``, ``db``, ``accounts``, ``jobs``, ``logins``,
``pool``, ``reporter`` (optional — omit when running without live progress).
"""
